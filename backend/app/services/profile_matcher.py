...
from app.db.models import Card, FixabilityFlag, GapCluster, RoleMatch, UserProfile, EvidenceItem
...
async def match_profile_and_write_cards(
    ...
    *,
    ...
    for cluster in clusters:
        # Layer 3: Require evidence_count >= 2. Single-item clusters are 1-off noise or isolated pages.
        if cluster.evidence_count < 2:
            logger.info(
                "Cluster '%s' skipped (insufficient evidence_count: %d < 2)",
                cluster.label, cluster.evidence_count
            )
            continue
        ...
        if flag.fixability_type == "too_vague":
            logger.info(
                "Cluster '%s' skipped (fixability_type=too_vague) | %s",
                cluster.label,
                getattr(flag, 'fixability_reason', 'no reason recorded'),
            )
            continue
        ...
        # Compute combined score: evidence strength dominates (0.7) over profile match (0.3)
        # cluster.rank_score is 0-100. sim is 0.0-1.0. 
        combined_score = (cluster.rank_score * 0.7) + ((sim * 100.0) * 0.3)
        ...
        scored_clusters.append((cluster, sim, combined_score, flag, best_role_match))
            ...
            async with db.begin_nested():
                ...
                if needs_generation:
                    ...
                    try:
                        ...
                        card.source_urls = []
                        card.fit_tier = "stretch"
                        card.bridge_note = "Error generating bridge note."
