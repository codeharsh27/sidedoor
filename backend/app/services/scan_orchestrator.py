...
async def scan_company(company_id: str, db: AsyncSession) -> dict:
    ...
    # 1. Fetch company with row-level lock
    ...
    try:
        ...
        if len(evidence_items) + existing_count < 5 and settings.twitter_bearer_token:
            try:
        ...
        if len(evidence_items) + existing_count < 3:
            logger.info("Insufficient signal for company %s (found %d items total)", company.name, len(evidence_items) + existing_count)
            company.scan_status = "insufficient_signal"
            company.last_scanned_at = now
            await db.commit()
            return {
                "status": "insufficient_signal",
                "evidence_count": len(evidence_items) + existing_count,
                "scan_status": company.scan_status,
                "last_scanned_at": company.last_scanned_at,
            }
        ...
        # Re-fetch company in case of session rollback or detached instance
        stmt = select(Company).where(Company.id == company_id).with_for_update()
        result = await db.execute(stmt)
        company = result.scalar_one_or_none()
        if company:
